...
const knowledgeBase = ragData as KnowledgeItem[];
  ...
  return knowledgeBase
    .map((item) => {
      ...
      const score = Array.from(queryTokens).reduce(
        (total, token) => total + (itemTokens.has(token) ? 1 : 0),
        0
      );
      return { item, score };
    })
...
export function formatKnowledgeContext(
  ...
  if (!items.length) {
    return 'No closely matching problem was found in the local knowledge base.';
  }
  return items
    .map(
      (item) =>
